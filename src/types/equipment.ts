...
// 현재 레벨의 장비 데이터 가져오기
export function getEquipmentAtLevel(
...
// 대장장이 코멘트 가져오기
export function getEquipmentComment(
  equipment: EquipmentBase,
  level: number
): string {
...
export function getStarForceBonus(
  ...
  if (baseStats.attack) {
    bonus.attack =
      Math.floor(baseStats.attack * starMultiplier) - baseStats.attack;
  }
  if (baseStats.defense) {
    bonus.defense =
      Math.floor(baseStats.defense * starMultiplier) - baseStats.defense;
  }
  if (baseStats.hp) {
...
const TOTAL_BASED_PERCENT_STATS: (keyof CharacterStats)[] = [
  "attack",
  "defense",
  "hp",
];
// 값 자체가 %인 스탯 (직접 더함)
const ADDITIVE_PERCENT_STATS: (keyof CharacterStats)[] = [
  "critRate",
  "critDamage",
  "penetration",
  "attackSpeed",
...
export function calculateEquipmentStats(
  ...
  const baseWithMultipliers: CharacterStats = {
    ...
    defense: Math.floor(
      (equipmentBase.baseStats.defense || 0) * starMultiplier
    ),
    ...
    evasion: equipmentBase.baseStats.evasion || 0, // 회피율은 스타포스로 증가하지 않음
  };
