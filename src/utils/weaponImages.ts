 ...
 */
export function getWeaponImage(weaponId: string, level: number = 0): string | undefined {
  ...
  const availableLevels = Object.keys(images).map(Number).sort((a, b) => b - a)
 ...
 * 무기의 모든 레벨 이미지 가져오기
 */
export function getWeaponImages(weaponId: string): Record<number, string> | undefined {
