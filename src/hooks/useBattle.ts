...
// 난이도별 레벨 차이 범위
const DIFFICULTY_LEVEL_RANGES: Record<
  AIDifficulty,
  { min: number; max: number }
> = {
  easy: { min: -3, max: -1 }, // 플레이어보다 1~3강 낮음
  normal: { min: -1, max: 1 }, // 플레이어와 비슷함 (±1강)
  hard: { min: 1, max: 3 }, // 플레이어보다 1~3강 높음
  extreme: { min: 3, max: 6 }, // 플레이어보다 3~6강 높음
};

function generateAIData(
  difficulty: AIDifficulty,
  playerStats: CharacterStats,
  playerWeapon: UserWeapon | null
): AIData {
  ...
  // 플레이어 무기 레벨 기준으로 AI 무기 레벨 계산
  const playerLevel = playerWeapon?.starLevel ?? 0;
  const levelRange = DIFFICULTY_LEVEL_RANGES[difficulty];
  const levelOffset = Math.floor(
    levelRange.min + Math.random() * (levelRange.max - levelRange.min + 1)
  );
  let level = Math.max(0, playerLevel + levelOffset);
  // 최대 레벨 제한 (30강)
  ...
  const stats: CharacterStats = {
    ...
    defense: Math.floor(
      (DEFAULT_CHARACTER_STATS.defense + playerStats.defense * 0.5) *
        config.multiplier
    ),
    hp: Math.floor(
      (DEFAULT_CHARACTER_STATS.hp + playerStats.hp * 0.3) * config.multiplier
    ),
    // 치명타 확률: 플레이어 치명타의 70% 반영 (상한 85%)
    critRate: Math.min(
      85,
      Math.floor(
        (DEFAULT_CHARACTER_STATS.critRate + playerStats.critRate * 0.7) *
          config.multiplier
      )
    ),
    // 치명타 데미지: 플레이어 초과분의 60% 반영
    critDamage: Math.floor(
      (DEFAULT_CHARACTER_STATS.critDamage +
        (playerStats.critDamage - 150) * 0.6) *
        config.multiplier
    ),
    // 관통력: 플레이어 관통력의 50% 반영
...
function calculateDamage(
  ...
  // 기본 대미지 (랜덤 요소 포함)
  const randomBonus =
    attack * BATTLE_CONFIG.randomFactor * (Math.random() * 2 - 1);
...
export function useBattle(
  ...
  const getExpectedReward = useCallback(
    (
      ...
      return {
        win: calculateBattleReward(
          "win",
          playerAttack,
          playerLevel,
          config.rewardMultiplier
        ),
        lose: calculateBattleReward(
          "lose",
          playerAttack,
          playerLevel,
          config.rewardMultiplier
        ),
        draw: calculateBattleReward(
          "draw",
          playerAttack,
          playerLevel,
          config.rewardMultiplier
  ...
  const startBattle = useCallback(
    async (
      ...
      // AI 생성 (무기 + 스탯)
      const aiData = generateAIData(difficulty, effectiveStats, playerWeapon);
      const aiName = getRandomAIName();

      // 카드 효과가 적용된 플레이어 스탯
      let boostedPlayerStats = applyCardBonusesToStats(
        effectiveStats,
        cardBonuses
      );
      ...
      const opponentDamageResult = calculateDamage(
        opponent.stats,
        player.stats
      );
      ...
      if (cardBonuses.damageReflect > 0) {
        const reflectedDamage = Math.floor(
          opponentDamage * cardBonuses.damageReflect
        );
      ...
      const finishedBattle: BattleMatch = {
        ...
        opponent,
        winner:
          result === "win" ? "player" : result === "lose" ? "opponent" : null,
        result,
        goldReward,
        status: "finished",
        finishedAt: new Date(),
