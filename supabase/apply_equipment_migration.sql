-- =============================================
-- 장비 시스템 마이그레이션 (Supabase SQL Editor에서 실행)
-- 다중 장비 + 스탯 + 잠재옵션 + PvP
-- =============================================
...
CREATE TABLE IF NOT EXISTS equipment_potentials (
  ...
  is_locked BOOLEAN NOT NULL DEFAULT FALSE,      -- 고정 여부 (리롤 시 유지)
  is_unlocked BOOLEAN NOT NULL DEFAULT FALSE,    -- 슬롯 해제 여부 (false면 비활성)
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE OR REPLACE TRIGGER update_user_equipment_updated_at
...
CREATE OR REPLACE TRIGGER update_pvp_rankings_updated_at
  BEFORE UPDATE ON pvp_rankings
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

CREATE OR REPLACE TRIGGER update_daily_pvp_updated_at
